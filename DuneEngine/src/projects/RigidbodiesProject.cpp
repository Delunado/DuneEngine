...
{
    Body* circle1 = new Body(CircleShape(40.0f), Vec2(400.0f, 300.0f), 5.0f);
    _bodies.push_back(circle1);

    Body* circle2 = new Body(CircleShape(20.0f), Vec2(500.0f, 300.0f), 5.0f);
...
void RigidbodiesProject::FixedUpdate()
{
    ...
    for (int i = 0; i < _bodies.size() - 1; i++)
    {
        ...
        for (int j = i + 1; j < _bodies.size(); j++)
        {
            ...
            if (CollisionDetection::IsColliding(body, otherBody))
            {
